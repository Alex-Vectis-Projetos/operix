...
describe("Spec 005 — normative acceptance synchronized through T05", () => {
  ...
  it("EXPENSE-CREATE-01 creates scoped effective expense", async () => { const response = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-create"), body: JSON.stringify(expense) }); expect(response.status).toBe(201); const item = await response.json(); expect(item).toMatchObject({ amount: "3000.00", status: "effective", idempotent: false, createdByUserId: fixture.ownerA }); expect((await prisma.expense.findUniqueOrThrow({ where: { id: item.id } })).workspaceId).toBe(fixture.workspaceA); });
  it("EXPENSE-DECIMAL-01 accepts decimal strings and rejects JSON number money", async () => { const decimal = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-decimal"), body: JSON.stringify({ ...expense, amount: "0.10" }) }); expect(decimal.status).toBe(201); expect((await decimal.json()).amount).toBe("0.10"); const numeric = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-numeric"), body: JSON.stringify({ ...expense, amount: 0.1 }) }); expect(numeric.status).toBe(422); });
  it("EXPENSE-LINKAGE-01 accepts only same-tenant tagged context", async () => { const linked = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-link"), body: JSON.stringify(expense) }); expect(linked.status).toBe(201); expect((await linked.json()).context).toEqual({ kind: "payment_list", id: fixture.eurPaidList }); const foreign = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-link-foreign"), body: JSON.stringify({ ...expense, context: { kind: "payment_list", id: fixture.bPaidList } }) }); expect(foreign.status).toBe(404); });
  it("EXPENSE-AUDIT-01 reverses effective history without delete", async () => { const created = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-audit-create"), body: JSON.stringify(expense) }); const item = await created.json(); const reversed = await request(`/expenses/${item.id}/reverse`, { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-audit-reverse"), body: JSON.stringify({ reason: "duplicate" }) }); expect(reversed.status).toBe(200); expect(await reversed.json()).toMatchObject({ id: item.id, status: "reversed", reversalReason: "duplicate", reversedByUserId: fixture.ownerA }); expect(await prisma.expense.count({ where: { id: item.id, status: "reversed" } })).toBe(1); });
  it("EXPENSE-TENANT-01 requires own success before foreign privacy assertion", async () => { expect((await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-tenant-own"), body: JSON.stringify(expense) })).status).toBe(201); const foreign = await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-tenant-foreign"), body: JSON.stringify({ ...expense, context: { kind: "payment_list", id: fixture.bPaidList } }) }); expect(foreign.status).toBe(404); });
  it("EXPENSE-IDOR-01 hides foreign expense identity", async () => { const foreignCreate = await request("/expenses", { method: "POST", headers: headers(fixture.ownerB, fixture.workspaceB, "expense-idor-b"), body: JSON.stringify({ ...expense, context: { kind: "payment_list", id: fixture.bPaidList } }) }); const foreign = await foreignCreate.json(); expect((await request(`/expenses/${foreign.id}`, { headers: headers(fixture.ownerA, fixture.workspaceA, "expense-idor-read") })).status).toBe(404); expect((await request(`/expenses/${foreign.id}/reverse`, { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-idor-reverse"), body: JSON.stringify({ reason: "no" }) })).status).toBe(404); });
  ...
  it("FIN-NO-SPEC004-MUTATION-01 preserves PaymentList commercial truth after successful Expense mutation", async () => { const before = await prisma.paymentList.findUniqueOrThrow({ where: { id: fixture.eurPaidList }, select: { status: true, recognizedTotal: true, sourceDocumentTotal: true } }); expect((await request("/expenses", { method: "POST", headers: headers(fixture.ownerA, fixture.workspaceA, "expense-spec004-guard"), body: JSON.stringify(expense) })).status).toBe(201); expect(await prisma.paymentList.findUniqueOrThrow({ where: { id: fixture.eurPaidList }, select: { status: true, recognizedTotal: true, sourceDocumentTotal: true } })).toEqual(before); });
});
