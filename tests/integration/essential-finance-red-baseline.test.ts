...
describe("Spec 005 — normative acceptance synchronized through T05", () => {
  ...
  it("FIN-CROSS-TENANT-01 proves own operation before foreign 404", async () => { expect((await ownerSummary()).status).toBe(200); expect((await request("/summary", { headers: headers(fixture.ownerA, fixture.workspaceB) })).status).toBe(404); });
