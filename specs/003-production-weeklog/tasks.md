...
| **T05** | **Authoritative Weeklog REST API & Submit Contract** | `DONE` | Migration forward-only mínima `20260917110000_spec_003_weeklog_validation_round_lifecycle` expandindo `WeeklogValidation` com ciclo de vida de rodada (`status: pending`, `submittedAt`, `submittedBy`, campos de chancela opcionais). Criação de `backend/src/routes/weeklogs.ts` com `GET /api/weeklogs`, `GET /api/weeklogs/:id`, `GET /api/weeklogs/:id/entries`, `GET /api/weeklogs/:id/entries/:entryId` e `POST /api/weeklogs/:id/submit-for-validation`. Isolamento estrito de tenant (`RequestContext`), filtragem server-side de técnico (`scope: own`), autoridade de submissão restrita a gestores/owners (bloqueio 403 de técnico), transição com lock pessimista (`SELECT ... FOR UPDATE`), retry idempotente na mesma rodada (HTTP 200), bloqueio de estados indevidos (HTTP 409), congelamento imutável de `coverageSnapshot` e pureza total de requisições GET (zero escritas). Todos os cenários T05 e 9 testes novos de hardening verdes (50 testes GREEN). |
