...
| **T01** | **Database Schema & Versioned Migration** | `DONE` | Adição de `Workspace.timezone`, `ClientAccessGrant` (com ciclo de vida: `status`, `grantedAt`, `revokedAt`, `revokedBy` e composite FK com `Client`), `ProductionOrder` (`executionSequence`, `operationalSiteKey`, `currencyCode`, `performedServices` e `rectificationOriginId`), `Weeklog` (`status: open/pending_validation/validated/rectification_pending`), `WeeklogEntry` (`executionSequence`, `legacyServiceOrderId`, `currencyCode` sem default EUR e `totalAmount`), e `WeeklogValidation` versionado (`validationSequence`, `coverageSnapshot`). Constraints `@@unique([productionOrderId, executionSequence])` e `@@unique([weeklogId, validationSequence])`. Migration `20260917000000_add_weeklog_canonical_domain_and_versioned_rectification` gerada e aplicada com zero drift e 100% de testes estruturais de constraints aprovados. |
| **T02** | **Test-First Suite (RED Invariants — 45 Cenários)** | `TODO` | Implementação de `tests/integration/weeklog-operational-flow.test.ts` cobrindo todos os **45 cenários BDD**: finalização atômica, lock pessimista, retificação na mesma semana (sequence 2), retificação em semanas subsequentes, cadeias múltiplas, segregação por cliente e local operacional, virada de DST, fallback de timezone, bloqueio estrito de auto-validação individual e em lote (`VALIDATOR-BATCH-SELF-01`), grants revogados (`VALIDATOR-REVOKED-01`), OPs diretas estruturadas com site e moeda obrigatórios (`DIRECT-OP-NO-SITE-01`, `FINALIZE-NO-CURRENCY-01`), submissão de lote (`SUBMIT-VALIDATION-01`, `SUBMIT-VALIDATION-IDEMPOTENT-01`), delegação de PATCH legado e imutabilidade de assinatura pós-validação. |
| **T03** | **Deterministic Week Calculation with Timezone** | `TODO` | Refatoração de `backend/src/lib/weekUtils.ts` com `date-fns-tz` para cálculo semanal determinístico (Domingo 00:00:00 a Sábado 23:59:59) respeitando `Workspace.timezone` (fallback universal `"UTC"`), convertendo timestamps para UTC com tratamento de viradas de ano e DST. |
| **T04** | **Atomic Finalization Service & Endpoint** | `TODO` | Implementação de `finalizeProductionOrder` em `weeklogService.ts` com `SELECT ... FOR UPDATE`, validação estrita de `clientId`, `operationalSiteKey`, `currencyCode` e `performedServices`, snapshot congelado, upsert de `Weeklog`, criação de `WeeklogEntry(sequence)`, Downstream Legacy Projection Adapter em `service_orders`, e tratamento de colisão `P2002` **fora da transação abortada** com re-leitura segura e distinção de target da colisão. Exposição de `POST /api/production-orders/:id/finalize`. |
