...
| **T08** | **Sanitize Legacy ServiceOrders, Downstream Legacy Projection Adapter & Backfill Script** | `DONE` | Refatoração profunda de `backend/src/routes/serviceOrders.ts` aplicando `RequestContext` e autenticação em nível de router, eliminando completamente escritas em `GET /api/service-orders` (pure read-only), isolamento estrito de tenant e escopo de técnico (`own`), descontinuação de criação/substituição direta (`POST` e `PUT` retornam 410 Gone), imutabilidade estrita de projeções canônicas de `WeeklogEntry` (409 Conflict), rejeição de validação legada sem geração de `PaymentOrder` (409 Conflict), e bloqueio de deleção de projeções/arquivo histórico (409 Conflict). Implementação do script seguro de backfill `backend/scripts/backfillLegacyServiceOrders.ts` com `--dry-run` padrão, checagens estritas de qualificação determinística (sem suposições ou overwrites) e zero efeitos financeiros colaterais. Suíte dedicada de integração com 14 testes GREEN (`tests/integration/service-orders-legacy-sanitization.test.ts`), suíte da Spec 003 99/99 GREEN e regressão 76/76 GREEN. |
